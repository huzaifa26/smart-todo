...
export default function TaskDetail({closeModalHandler,task,detailModal}) {
  ...
  return (
    <div style={detailModal===true?{transform:"scale(1)",opacity:"1"}:{transform:"scale(0)",opacity:"0"}} className='w-[100vw] transition-opacity duration-150 absolute z-10 top-0 left-0 bg-[rgba(0,0,0,0.25)] backdrop-blur-sm h-[100vh] flex justify-center items-center'>
      <div style={detailModal===true?{transform:"scale(1)",opacity:"1"}:{transform:"scale(0)",opacity:"0"}} className='w-[70%] transition-opacity duration-150 h-[80%] bg-white rounded-lg p-4 divide-y-2 space-y-4'>
        ...
        <div className='p-2 space-y-2 pt-4'>
          ...
          <h2><span className='text-lg font-bold'>Last Updated: </span>{new Date(task?.last_updated).toISOString()}</h2>

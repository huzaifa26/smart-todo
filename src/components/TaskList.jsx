  ...
  return (
    <div onClick={() => openModalHandler(task)} key={task.id} style={{ boxShadow: "0px 0px 4px rgba(0,0,0,0.1)" }} className='h-[80px] rounded-lg p-2 px-4 w-full flex justify-between items-center transition-transform cursor-pointer relative bg-[white]'>
      <div>
        ...
        <p className="truncate w-[38vw] text-sm">{task.description}</p>
      </div>
      <div className='flex items-center gap-2 relative'>
        ...
        {task?.isMissed === true &&
          <button onClick={(e) => e.stopPropagation()} disabled={disableStart} style={task?.isMissed ? { opacity: "0.5" } : {}} className='m-2 bg-[#0E123F] text-white rounded-lg w-24 h-10 transition-colors'>Missed</button>
        }
